...
首先，编译器会尝试定位表示导入模块的文件。 编译器会遵循以下二种策略之一： [Classic](https://www.tslang.cn/docs/handbook/module-resolution.html#classic)或[Node](https://www.tslang.cn/docs/handbook/module-resolution.html#node)。 这些策略会告诉编译器到 _哪里_去查找`moduleA`。
...
利用`rootDirs`，可以告诉编译器生成这个虚拟目录的_roots_； 因此编译器可以在“虚拟”目录下解析相对模块导入，就 _好像_它们被合并在了一起一样。
